    ...
    <div class="comment-list">
      ...
      >
        ...
        <div class="comment-main">
          ...
          <div class="comment-content">
            <div class="comment-meta">
              ...
              <span v-if="comment.userId == userId" class="user-tag"
                >我自己</span
              >
            </div>
            ...
            <div class="opr">
              ...
              <div class="btn">
                ...
                <div @click="toggleReplyInput(comment)">
                  ...
                  <span>{{
                    !comment.showReplyInput ? "回复" : "取消回复"
                  }}</span>
                </div>
                <div class="upvote" @click="upvote(comment)">
                  <img
                    v-if="!comment.upvoteFlag"
                    src="/update-active.png"
                    alt=""
                  />
                  <img v-else src="/update-default.png" alt="" />
                  <span
                    class="count"
                    :class="{ animate: upvoteAnimate === comment.id }"
                  >
        ...
        <div class="child-comments">
          <div
            v-for="(child, childIndex) in comment.commentChildVOS"
            :key="childIndex"
            class="child-comment"
          >
            <div class="comment-main">
              ...
              <div class="comment-content">
                <div class="comment-meta">
                  ...
                  <span v-if="child.userId == userId" class="user-tag"
                    >我自己</span
                  >
                  <span v-if="child.replierName != null" class="reply-to">
                    回复
                    <img :src="child.replierAvatar" class="replier-avatar" />
                    {{ child.replierName }}
                    <span v-if="child.replierId == userId" class="user-tag"
                      >我自己</span
                    >
                  </span>
                </div>
                ...
                <div class="opr">
                  ...
                  <div class="btn">
                    ...
                    <div @click="toggleChildReplyInput(child)">
                      <i class="el-icon-chat-round"></i>
                      <span>{{
                        !child.replyInputStatus ? "回复" : "取消回复"
                      }}</span>
                    </div>
                    <div class="upvote" @click="upvote(child)">
                      <img
                        v-if="!child.upvoteFlag"
                        src="/update-active.png"
                        alt=""
                      />
                      <img v-else src="/update-default.png" alt="" />
...
export default {
  ...
  methods: {
    upvote(comment) {
      ...
      this.$axios
        .post(`evaluations/upvoteOperation`, { id: comment.id })
        .then(res => {
          if (res.code == 200) {
            let rep = res.data;
    ...
    closeOtherReplyInputs(currentCommentId) {
      ...
      this.commentList.forEach(parentComment => {
        if (
          parentComment.id !== currentCommentId &&
          parentComment.showReplyInput
        ) {
        ...
        if (parentComment.commentChildVOS) {
          parentComment.commentChildVOS.forEach(childComment => {
            if (
              childComment.id !== currentCommentId &&
              childComment.replyInputStatus
            ) {
