        ...
        <el-tab-pane label="核心信息" name="first">
          <div>
            ...
            <div>
              <p>*用户账号</p>
              <span
                v-if="!dialogControlOperation"
                style="display: inline-block;margin-bottom: 10px;font-size: 10px;"
                >账号一经注册，不可修改</span
              >
              <el-input
                :disabled="!dialogControlOperation"
                placeholder="输入"
  ...
  methods: {
    ...
    async updateUserOperation() {
      try {
        ...
        this.apiParam.password = !this.password
          ? null
          : this.$md5(this.$md5(this.password)); // 没输入就是不用改，否则就是密码二次md5加密
