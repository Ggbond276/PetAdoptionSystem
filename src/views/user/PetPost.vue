<template>
  <div class="pet-post">
    ...
    <div class="pet">
      <div class="pet-type">
        <div
          :class="{ active: petTypeId === petType.id }"
          @click="petTypeClick(petType)"
          class="pet-type-item"
          v-for="petType in petTypeList"
          :key="petType.id"
        >
          {{ petType.name }}
        </div>
      ...
      <div class="pet-list">
        ...
        <div>
          <div
            @click="goDetail(petPost)"
            class="pet-post-item"
            v-for="(petPost, index) in petPostList"
            :key="index"
          >
            <div style="display: flex;gap: 10px;box-sizing: border-box;">
              <div>
                <img
                  style="width: 220px;height: 140px;border-radius: 5px;"
                  :src="petPost.cover"
                  alt=""
                />
              </div>
              <div>
                ...
                <div style="display: flex;gap: 10px;font-size: 12px;">
                  <span> 点赞量({{ petPost.upvoteNumber || 0 }}) </span>
                  <span> 浏览量({{ petPost.viewNumber || 0 }}) </span>
                  <span> 收藏量({{ petPost.saveNumber || 0 }}) </span>
                </div>
              </div>
            </div>
          </div>
