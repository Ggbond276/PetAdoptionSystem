<template>
  <div class="container">
    ...
    <div>
      <el-table :data="apiResult.data">
        ...
        <el-table-column prop="avatar" width="180" label="领养人基本信息">
          <template #default="scope">
            <div
              style="display: flex;justify-content: left;align-items: center;gap: 10px;"
            >
              <img
        ...
        <el-table-column prop="status" width="120" label="订单状态">
          <template #default="scope">
            ...
            <el-tag
              ...
              >已审核</el-tag
            >
            <el-tag size="mini" type="danger" v-else-if="scope.row.status === 3"
              >审核不通过</el-tag
            >
            <el-tag tsize="mini" ype="success" v-else>已完成</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="createTime"
                ...
                <el-dropdown trigger="click" placement="bottom-end">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item
                      ...
                      @click.native="againReply(scope.row)"
                      icon="el-icon-c-scale-to-original"
                      >重新提交证明材料</el-dropdown-item
                    >
                    <el-dropdown-item
      ...
      <div class="pager">
        <el-pagination
          ...
          :current-page="petAdoptOrderQueryDto.current"
          :page-sizes="[10, 20]"
          :page-size="petAdoptOrderQueryDto.size"
    ...
    >
      <div style="margin: 10px 20px;">
        ...
        >
          <span
            >提交状态：{{
              petAdoptOrder.isAgainPost ? "再次提交" : "初次提交"
            }}</span
          >
        ...
        >
          {{ petAdoptOrder.addressee }} - {{ petAdoptOrder.concatPhone }} -
          {{ petAdoptOrder.detailAddress }}
        </div>
        <h4>证明材料</h4>
        <div
          style="padding: 10px;background-color: rgb(248,248,248);"
          v-html="petAdoptOrder.detail"
        ></div>
        <div
          v-if="petAdoptOrder.status === 3"
        ...
        <!-- 审核不通过原因框 -->
        <div style="margin-block: 10px;" v-if="petAdoptOrder.status === 3">
          <h4>原因补充</h4>
          <el-input
            type="textarea"
            :rows="2"
            placeholder="请输入内容"
            v-model="auditErrorDetail"
          >
          </el-input>
        </div>
      </div>
    ...
    <el-dialog
      title="证明材料补充提交"
      :show-close="false"
      :visible.sync="dialogAgainVisible"
      width="40%"
    >
      <div>
        <Editor
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="primary" @click="confirmAgainReply"
          >确定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    async fetchFreshData() {
      try {
        const { data, total } = await this.$axios.post(
          "/pet-adopt-order/queryUser",
          this.petAdoptOrderQueryDto
        );
    ...
    async confirmDeleted() {
      try {
        const { code } = await this.$axios.delete(
          `/pet-adopt-order/${this.id}`
        );
        if (code === 200) {
          this.$notify.success({
            title: "宠物领养订单删除",
